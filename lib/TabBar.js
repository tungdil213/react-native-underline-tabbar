...
class TabBar extends Component<Props, State> {
  ...
  onTabLayout(event: Object, page: number) {
    ...
    const firstItem = this?.tabState[0]?.width || 0 ;
    const filtered = Object.keys(this.tabState)
    .filter(key => key < page)
      .reduce((obj, key) => {

        obj= (parseInt(obj) || firstItem) + this.tabState[key].width;
        return obj;
      }, {});
    this.tabState[page] = { x:filtered, y, width, height };
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          isTabActive && { borderBottomColor: "white", borderBottomWidth: 2},
        ]}
        key={page}
